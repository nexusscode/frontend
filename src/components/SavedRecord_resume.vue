            ...
            <div>
                ...
                <div v-else>
                    <div v-if="titles.length > 0" class="mt-10 flex-grow grid grid-cols-5 gap-y-14 justify-items-start">
                        ...
                        >
                            <button 
                                @click="gotoResult(item.resumeId)"
                            ...
                            <span 
                                ...
                                :title="item?.companyName + ' - 자기소개서'"
                            >
                                {{ item?.companyName }} - 자기소개서
                            </span>
                        </div>
                    </div>
            ...
            <div class="relative h-10 flex items-end w-full">
                ...
                <div class="absolute left-1/2 transform -translate-x-1/2 flex">
                    ...
                    <button
                        ...
                        :key="page"
                        @click="currentPage = page; isLoading = true; fetchSResume(page)"
...
async function fetchSResume(page = 1) { // 보관된 자소서 전체 조회
    try {
        const response = await env.get('/api/application/resume/saved', {
            params: {
        ...
        console.log("보관된 자소서 전체 조회:" + titles.value)
...
onMounted(fetchSResume);

function gotoResult(resumeId) {
...
async function handleSearch() {
    ...
    if (keyword === '') {
        ...
        // titles.value = titles.value.filter(t =>
